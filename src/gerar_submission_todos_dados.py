...
df_completo = pd.read_csv(caminho_treino)
df_teste = pd.read_csv(caminho_teste)

# Validacao cruzada usando todas as linhas originais, sem remocao de anomalias.
...
for numero_fold, (indices_treino, indices_validacao) in enumerate(
    validacao_cruzada.split(df_completo),
    start=1,
):
    df_treino_fold = df_completo.iloc[indices_treino]
    df_validacao_fold = df_completo.iloc[indices_validacao]
...
# Treinamento final com as 4.683 linhas.
...
x_completo = df_modelo.drop(columns=["Id", "preco"])
y_completo = df_modelo["preco"]
...
caminho_saida = os.path.join(
    pasta_saida,
    "submission_lightgbm_categorico_800_todos_dados.csv",
)
...
print(f"\nLinhas limpas: {len(df_limpo)}")
print(f"Linhas neste experimento: {len(df_completo)}")
print(f"Linhas recuperadas: {len(df_completo) - len(df_limpo)}")
